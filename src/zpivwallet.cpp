...
CzZEFIRWallet::CzZEFIRWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZZEFIRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZZEFIRSeed_deprecated()) {
                    LogPrintf("%s: Updated zZEFIR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzZEFIRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZZEFIRCount(nCountLastUsed);
    else if (!walletdb.ReadZZEFIRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzZEFIRWallet::Lock()
{
    seedMaster = 0;
}

void CzZEFIRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzZEFIRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZZEFIR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzZEFIRWallet::LoadMintPoolFromDB()
{
...
void CzZEFIRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzZEFIRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzZEFIRWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzZEFIRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZZEFIR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzZEFIRWallet::SeedToZZEFIR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzZEFIRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzZEFIRWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZZEFIRCount(nCountLastUsed);
...
void CzZEFIRWallet::GenerateDeterministicZZEFIR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzZEFIRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzZEFIRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

...
public:
    ...
    bool WriteZZEFIRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZZEFIRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZZEFIRSeed_deprecated(uint256& seed);
    bool EraseZZEFIRSeed();
    bool EraseZZEFIRSeed_deprecated();

    bool WriteZZEFIRCount(const uint32_t& nCount);
    bool ReadZZEFIRCount(uint32_t& nCount);

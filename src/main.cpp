...
#if defined(NDEBUG)
#error "ZEFIR cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zZEFIR is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zZEFIR spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zZEFIR without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zZEFIR spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zZEFIR spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zZEFIR spend tx %s already in block %d",
            ...
            // Check that zZEFIR mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 51){ //PREMINE 1 MILLION
        nSubsidy = 20000 * COIN;
    //} else if (nHeight == 245005) {
    //     nSubsidy = 10 * 1000 * 1000 * COIN; //RESTART PROJECT FEE || 10'000'000 Zefir
    } else {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZZEFIRStake)
{
    int64_t ret = 0;
    ...
    // } else {
    //     //When zZEFIR is staked, masternode only gets 2 ZEFIR
    //     ret = 3 * COIN;
    //     if (isZZEFIRStake)
    //         ret = 2 * COIN;
    // }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ZEFIR
...
{
    RenameThread("zefir-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZZEFIRMinted()
{
...
void RecalculateZZEFIRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zZEFIR supply
...
bool RecalculateZEFIRSupply(int nHeightStart)
{
...
{
    // ZEFIR: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZZEFIRSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZZEFIRMinted();
        RecalculateZZEFIRSpent();
        RecalculateZEFIRSupply(Params().Zerocoin_StartHeight());
    }

    //Track zZEFIR money supply in the block index
    if (!UpdateZZEFIRSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zZEFIR supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zZefirSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ZEFIR to zZEFIR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zZEFIR spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zZEFIR serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zZEFIR stake block is less than allowed start height", __func__);

    if (CZZefirStake* zZEFIR = dynamic_cast<CZZefirStake*>(stake)) {
        CBlockIndex* pindexFrom = zZEFIR->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zZEFIR stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zZEFIR stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zZEFIR->GetValue()));
        if (nChecksum200 != zZEFIR->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zZEFIR->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZZEFIR() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zZEFIR fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zZEFIR mints and %d zZEFIR spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        //@DVN EXAMPLE
        // ZEFIR: We use certain sporks during IBD, so check to see if they are

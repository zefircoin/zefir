...
#ifndef ZEFIR_ZZEFIRWALLET_H
#define ZEFIR_ZZEFIRWALLET_H
...
public:
    CzZEFIRWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZZEFIR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZZEFIR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ZEFIR_ZZEFIRWALLET_H

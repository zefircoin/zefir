...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ZEFIR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zZEFIR send
enum ZerocoinSpendStatus {
    ZZEFIR_SPEND_OKAY = 0,                            // No error
    ZZEFIR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZZEFIR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZZEFIR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZZEFIR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZZEFIR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZZEFIR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZZEFIR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZZEFIR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZZEFIR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZZEFIR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZZEFIR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZZEFIR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZZEFIR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZZEFIR_SPENT_USED_ZZEFIR = 14,                      // Coin has already been spend
    ZZEFIR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZZEFIR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZZefirBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZZEFIROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzZEFIRWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzZEFIRTracker> zpivTracker;
    ...
    void setZWallet(CzZEFIRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzZEFIRTracker>(new CzZEFIRTracker(strWalletFile));
    }

    CzZEFIRWallet* getZWallet() { return zwalletMain; }
    ...
    void setZZefirAutoBackups(bool fEnabled)
    {
    ...
    /** zZEFIR reset */
    boost::signals2::signal<void()> NotifyzZEFIRReset;

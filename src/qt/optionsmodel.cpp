...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZEFIR);
    ...
    if (!settings.contains("nAnonymizeZefirAmount"))
        settings.setValue("nAnonymizeZefirAmount", 1000);

    nAnonymizeZefirAmount = settings.value("nAnonymizeZefirAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZefirAmount"))
        SoftSetArg("-anonymizezefiramount", settings.value("nAnonymizeZefirAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zefir.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZefirAmount:
            return QVariant(nAnonymizeZefirAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZefirAmount:
            nAnonymizeZefirAmount = value.toInt();
            settings.setValue("nAnonymizeZefirAmount", nAnonymizeZefirAmount);
            emit anonymizeZefirAmountChanged(nAnonymizeZefirAmount);
            break;
        case CoinControlFeatures:

...
std::set<std::string> ZZefirControlDialog::setSelectedMints;
std::set<CMintMeta> ZZefirControlDialog::setMints;

ZZefirControlDialog::ZZefirControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZZefirControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZZefirControlDialog::~ZZefirControlDialog()
{
    delete ui;
}

void ZZefirControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZefirControlDialog::updateList()
{
...
void ZZefirControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZefirControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZZefir_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZefirControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZZefirControlDialog::GetSelectedMints()
{
...
void ZZefirControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
